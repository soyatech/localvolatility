...
class volatility_surface():
    ...
    def load_data(self):
        option = data.Options(self.quote, 'yahoo')

        originalData = option.get_all_data()
        self.underlying_price = originalData.iloc[0].Underlying_Price
        self.quote_time = originalData.iloc[0].Quote_Time
        self.optionData = originalData
        self.optionData.sort(inplace=True)

        itmCall = self.optionData.loc[(slice(None, self.underlying_price * 0.9), slice(None))].loc[(slice(None), slice(None), 'call'), :]
        itmPut = self.optionData.loc[(slice(self.underlying_price*1.1, None),slice(None))].loc[(slice(None),slice(None),'put'),:]
        self.optionData.drop(itmCall.index,inplace =True)
        self.optionData.drop(itmPut.index, inplace=True)
        self.optionData["Iv"] = self.optionData.apply(lambda f: self.p2f(f["IV"]), axis=1)
        self.expiries = self.optionData.index.get_level_values("Expiry").unique().date
        self.result = [e for e in self.expiries if e > self.quote_time.date()]
        self.result.sort()
    ...
    def __day_to_maturity(self, expiry):
        total_seconds = ((expiry - self.quote_time.date()).total_seconds())
    ...
    def show_implied_volatility_surface(self):
        surfaceData = self.get_all_smiles(self.result)
    ...
    def show_smooth_iv_surface(self):
        ...
        band = 0.2
        tr = (0.05, 1)
        xi = np.linspace(self.underlying_price * (1 - band),
                         self.underlying_price * (1 + band), 50)
        yi = np.arange(tr[0], tr[1], 7 / 365)

        X, Y = np.meshgrid(xi, yi)
        fitfunc = np.vectorize(__fitted_iv)
        Z = fitfunc(self.underlying_price, X, Y)
        ...
        for index, row in fittedSurface.iterrows():
            for col in fittedSurface.columns:
                row[col] = __fitted_iv(self.underlying_price, index, col)
    ...
    def cal_local_volatility(self, vol, T, t1, k, k1, k2, vol_k_1,
                            vol_k_2, vol_t_1):
        s = self.underlying_price
        r = 0.01
    ...
    def local_voltility(self, ivS):
        ...
        for i in range(1, len(mats)):
            ...
            for j in range(2, len(ks)):
                ...
                localSurface.iloc[j - 2, i - 1] = self.cal_local_volatility(vol, T, t1, k, k1, k2, vol_k_1, 
                                                             vol_k_2, vol_t_1)
        return localSurface
